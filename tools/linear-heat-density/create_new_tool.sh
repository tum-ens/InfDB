...
# Replace all occurrences of "linear-heat-density" with the new name
echo "Replacing placeholders with $NEW_NAME..."
find "$TARGET_DIR" -type f -exec sed -i.bak "s/linear-heat-density/$NEW_NAME/g" {} +
find "$TARGET_DIR" -type f -exec sed -i.bak "s/linear_heat_density/$SNAKE_CASE_NAME/g" {} +
find "$TARGET_DIR" -type f -name "*.bak" -delete

# Rename files that contain "linear-heat-density" or "linear_heat_density"
find "$TARGET_DIR" -depth -name "*linear-heat-density*" -o -name "*linear_heat_density*" | while read -r file; do
    new_file=$(echo "$file" | sed "s/linear-heat-density/$NEW_NAME/g" | sed "s/linear_heat_density/$SNAKE_CASE_NAME/g")
    mv "$file" "$new_file"
done
