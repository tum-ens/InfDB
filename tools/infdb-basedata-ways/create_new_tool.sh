...
# Replace all occurrences of "infdb-basedata-ways" with the new name
echo "Replacing placeholders with $NEW_NAME..."
find "$TARGET_DIR" -type f -exec sed -i.bak "s/infdb-basedata-ways/$NEW_NAME/g" {} +
find "$TARGET_DIR" -type f -exec sed -i.bak "s/choose_a_name/$SNAKE_CASE_NAME/g" {} +
find "$TARGET_DIR" -type f -name "*.bak" -delete

# Rename files that contain "infdb-basedata-ways" or "choose_a_name"
find "$TARGET_DIR" -depth -name "*infdb-basedata-ways*" -o -name "*choose_a_name*" | while read -r file; do
    new_file=$(echo "$file" | sed "s/infdb-basedata-ways/$NEW_NAME/g" | sed "s/choose_a_name/$SNAKE_CASE_NAME/g")
    mv "$file" "$new_file"
done
