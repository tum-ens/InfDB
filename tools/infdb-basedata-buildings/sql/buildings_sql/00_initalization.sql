...
BEGIN
    ...
    IF got_lock THEN
        ...
        IF NOT EXISTS (
            ...
            -- Function 3: surface_area_corrected_geom
            BEGIN
                -- ============================================================
                -- Function: {output_schema}.surface_area_corrected_geom(geom, nx, ny, nz)
                -- Purpose:
                --   - True surface area of a single (possibly tilted, possibly holed)
                --     3D face, WITHOUT SFCGAL / CG_3DArea.
                -- Method ("lay the tilted face flat and measure its shadow"):
                --   - The caller provides the face's outward normal (nx, ny, nz),
                --     computed set-based via Newell's method in 04_a.
                --   - Rotate the face so its normal points up (face -> horizontal),
                --     ST_Force2D, then ST_Area.
                --   - ST_Area subtracts interior rings (holes) automatically, so
                --     courtyard footprints are correct (no more exploding areas), and
                --     vertical walls / sloped roofs keep their true 3D area (plain
                --     ST_Force2D would collapse them to ~0).
                -- Safety:
                --   - Degenerate/missing normal (collinear points) -> flat 2D area.
                --   - EXCEPTION -> 0.0 so a single bad face never crashes the query.
                -- ============================================================
                CREATE OR REPLACE FUNCTION {output_schema}.surface_area_corrected_geom(
                    geom geometry,
                    nx double precision,
                    ny double precision,
                    nz double precision
                ...
                BEGIN
                    IF geom IS NULL THEN RETURN 0.0; END IF;

                    -- Degenerate / missing normal (collinear vertices) -> flat 2D area.
                    IF nx IS NULL OR ny IS NULL OR nz IS NULL
                       OR (nx*nx + ny*ny + nz*nz) < 1e-20 THEN
                        RETURN ROUND(COALESCE(ST_Area(ST_Force2D(geom)), 0.0)::numeric, 3)::double precision;
                    END IF;

                    -- Rotate the face so its normal aligns with Z (face becomes
                    -- horizontal), flatten, measure. atan2 is scale-invariant so the
                    -- raw normal components can be used directly.
                    projected := ST_RotateY(
                        ST_RotateX(geom, atan2(ny, nz)),
                        -atan2(nx, sqrt(ny*ny + nz*nz))
                    );
                    result_area := ST_Area(ST_Force2D(projected));

                    RETURN ROUND(COALESCE(GREATEST(result_area, 0.0), 0.0)::numeric, 3)::double precision;
                EXCEPTION WHEN OTHERS THEN
                    RETURN 0.0;
                END;
                $func$ LANGUAGE plpgsql IMMUTABLE;

                RAISE NOTICE '[Init] ✓ Created surface_area_corrected_geom function';
            EXCEPTION WHEN OTHERS THEN
                IF SQLERRM LIKE '%concurrently updated%' THEN
                    RAISE NOTICE '[Init] ✓ surface_area_corrected_geom created by another worker';
                ELSE
                    RAISE WARNING '[Init] Error creating surface_area_corrected_geom: %', SQLERRM;
                END IF;
            END;
            ...
            BEGIN
                ...
                SELECT
                    ...
                    bs.objectclass_id,
                    bs.surface_type AS classname,  -- fortiss building_surface uses surface_type; keep column name classname for downstream
                    bs.area,
