...
# Replace all occurrences of "infdb-metadata" with the new name
echo "Replacing placeholders with $NEW_NAME..."
find "$TARGET_DIR" -type f -exec sed -i.bak "s/infdb-metadata/$NEW_NAME/g" {} +
find "$TARGET_DIR" -type f -exec sed -i.bak "s/infdb_metadata/$SNAKE_CASE_NAME/g" {} +
find "$TARGET_DIR" -type f -name "*.bak" -delete

# Rename files that contain "infdb-metadata" or "infdb_metadata"
find "$TARGET_DIR" -depth -name "*infdb-metadata*" -o -name "*infdb_metadata*" | while read -r file; do
    new_file=$(echo "$file" | sed "s/infdb-metadata/$NEW_NAME/g" | sed "s/infdb_metadata/$SNAKE_CASE_NAME/g")
    mv "$file" "$new_file"
done
